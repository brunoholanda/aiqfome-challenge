...
export default function ItemClient() {
  ...
  const toggleArrayValue = (value: string, arr: string[], limit = Infinity): string[] => {
...
const item = itemId
  ? allItems.find((i) => i.name.toLowerCase() === decodeURIComponent(itemId).toLowerCase())
  : null;
...
const total = useMemo(() => {
  ...
  const drinksExtra = Object.entries(drinkQty).reduce(
    (sum, [id, qty]) => sum + (DRINKS.find((d) => d.id === id)?.price ?? 0) * qty,
    0
  );
  ...
  const singleTotal = item.price + sizeExtra + drinksExtra + extrasExtra + utensilsExtra;
  return singleTotal * quantity;
}, [item, size, drinkQty, extras, utensil, quantity]);

if (!id || !itemId || !item) {
  return <p style={{ padding: 20 }}>Página inválida ou item não encontrado.</p>;
}

  const handleFinish = () => {
    if (quantity <= 0 || !item) return;
    ...
    router.push(`/store/${id}/cart`);
  };

  if (!item) return <p style={{ padding: 20 }}>Item não encontrado.</p>;
